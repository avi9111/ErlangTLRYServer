...
-export([reds/1, queue/1, memory/1]).

-export([eprof_start/0, eprof_stop/0]).

-export([fprof_start/0, fprof_start/1, fprof_stop/0, fprof_analyze/0]).

-export([cpu_etop/1, mem_etop/1, etop_stop/0]).
-define(APPS, [kernel, vtnemo]).
...
help() ->
    ...
    "~p:eprof_start() -> start eprof on all available pids~n"
    "~p:eprof_stop() -> stop eprof and print result~n"
    "~n~n"
    "DO NOT use on production system!~n"
    "~p:fprof_start() -> start fprof on all available pids~n"
    "~p:fprof_start(N) -> start and run fprof for N seconds~n"
    "~p:fprof_stop() -> stop eprof and print formatted result~n"
    "~p:fprof_analyze() to analyze collected statistics and "
    "print formatted result; use on production system with CARE~n"
    "~n~n"
    "~p:help() -> print this help~n", lists:duplicate(10, ?MODULE)).
            ...
            (P, Acc) ->
                case catch process_info(
                    ...
                    [{_, Len}, {_, Memory}, {_, Reds}, {_, Dict}, {_, CurFun}, {_, RegName}] ->
                        ...
                        if
                            Type == queue andalso Len == 0 andalso IntQLen == 0 ->
                                Acc;
                            true ->
...
dump(N, Rs) ->
    lists:foreach(fun({_, MsgQLen, Memory, Reds, Dict, CurFun, Pid, RegName}) ->
        io:format("** pid(~s)~n"
        "** registered name: ~p~n"
        "** memory: ~p~n"
        "** reductions: ~p~n"
...
%进程CPU占用排名
cpu_etop(InterVal) ->
    spawn(fun() -> etop:start([{output, text}, {interval, InterVal}, {lines, 20}, {sort, reductions}]) end).
